...
date: 2024-01-17 20:03:00 +09:00
...
## **풀이**
~~*일단, 한붓그리기 문제를 구현하는 것인지 고민했다.<br>그렇다면, 한붓그리기의 특성 상, 싸이클이 만들어진다는 것은 모든 점에 연결된 간선이 2개여야 한다는 것을 뜻한다.*~~

이 방식으로 구현하면 다음과 같은 반례가 발생한다.
```
0--1--3     0-1-2 간의 싸이클이 발생했음에도 1에 연결된 간선이 3개가 되기 때문
| /
|/
2
```
